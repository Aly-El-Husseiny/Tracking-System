...
namespace Cutting
{
    public partial class frmAris : Form
    {
        ...
        private void RefreshAris()
        {
            DGV_Aris_todo.DataSource = "";
            //depname = trackdb.Departments.Single(IDP => IDP.Departmet_ID == id.Recived_From)

            var aris = from id in trackdb.Tracks
            ...
            DGV_Aris_todo.Columns[5].DefaultCellStyle.BackColor = Color.Pink;
            for (int i = 0; i < DGV_Aris_todo.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_Aris_todo.Rows[i].DefaultCellStyle.BackColor = Color.LightBlue;
                }
            }
            ...
            for (int i = 0; i < DGV_Aris_todo.Rows.Count; ++i)
            {
                sumqty_grind += Convert.ToInt32(DGV_Aris_todo.Rows[i].Cells[5].Value);
            ...
            for (int i = 0; i < DGV_Aris_todo.Rows.Count; ++i)
            {
                int qty = Convert.ToInt32(DGV_Aris_todo.Rows[i].Cells[5].Value);
                double w = Convert.ToDouble(DGV_Aris_todo.Rows[i].Cells[2].Value);
                double h = Convert.ToDouble(DGV_Aris_todo.Rows[i].Cells[3].Value);
                double raw1 = (qty * w * h) / 1000000;
                sumArea_grind += Convert.ToDouble(raw1);

            }
            txt_tarea_Aris.Text = sumArea_grind.ToString();
        }
        private void frmAris_Load(object sender, EventArgs e)
        {
        ...
        private void txt_Aris_Man_KeyPress(object sender, KeyPressEventArgs e)
        {
            Utility.onlynumber(e);
        }

        private void txt_Aris_prodT_KeyPress(object sender, KeyPressEventArgs e)
        {
            Utility.onlynumber(e);
        }

        private void txt_Aris_downT_KeyPress(object sender, KeyPressEventArgs e)
        {
            Utility.onlynumber(e);
        }
        ...
        private void txt_qtysend_Aris_TextChanged(object sender, EventArgs e)
        {

        }

        private void txt_qtysend_Aris_KeyPress(object sender, KeyPressEventArgs e)
        {
            Utility.onlynumber(e);
        }

        private void btnAris_Click(object sender, EventArgs e)
        {
            if (txt_qtysend_Aris.Text == "" || txt_oc_Aris.Text == "" || comboSV.Text == "" || comboQC.Text == "" || comboQA.Text == "")
            { MessageBox.Show(@"من فضلك ادخل الكمية المرسلة أو قم باختيار القسم"); }
            ...
            else
            {
                ...
                {
                    if (up.QTY_ToDo < int.Parse(txt_qtysend_Aris.Text))
                    { MessageBox.Show(@"الكمية المدخلة أكبر من المطلوب"); }
                    else
                    {
                        up.QTY_ToDo = up.QTY_ToDo - int.Parse(txt_qtysend_Aris.Text);
                        up.QTY_Send = int.Parse(txt_qtysend_Aris.Text);
                        ////////// Send to Next ///////////////////
                        ...
                        add.QTY_Recive = int.Parse(txt_qtysend_Aris.Text);
                        add.QTY_ToDo = int.Parse(txt_qtysend_Aris.Text);
        ...
        private void btnFinishAris_Click(object sender, EventArgs e)
        {
            var track = from id in trackdb.Tracks
                        join idd in trackdb.GlassTypes on id.Glass_ID equals idd.Glass_ID
                        join iddd in trackdb.ITEMs on new { id.OC_ID, id.Item_ID, id.Pos } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos }
                        join idDEP in trackdb.Departments on id.Departmet_ID equals idDEP.Departmet_ID
                        where (id.Date <= dateToAris.Value && id.Date >= datefromAris.Value) && id.Recived_From == 13
                        select new
                        {
            ...
            DGV_Aris_Detail.DataSource = track;

            for (int i = 0; i < DGV_Aris_Detail.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_Aris_Detail.Rows[i].DefaultCellStyle.BackColor = Color.LightBlue;
                }
            }
        ...
        private void copyAlltoClipboard()
        {
            ...
            DGV_Aris_Detail.ClipboardCopyMode = DataGridViewClipboardCopyMode.EnableAlwaysIncludeHeaderText;
            DGV_Aris_Detail.MultiSelect = true;
            DGV_Aris_Detail.SelectAll();
            DataObject dataObj = DGV_Aris_Detail.GetClipboardContent();
            if (dataObj != null)
                Clipboard.SetDataObject(dataObj);

        }
        private void btnExcelAris_Click(object sender, EventArgs e)
        {
            copyAlltoClipboard();
        ...
        private void DGV_Aris_todo_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            txt_oc_Aris.Text = DGV_Aris_todo.SelectedRows[0].Cells[0].Value.ToString();
            txt_item_Aris.Text = DGV_Aris_todo.SelectedRows[0].Cells[1].Value.ToString();
            txt_w_Aris.Text = DGV_Aris_todo.SelectedRows[0].Cells[2].Value.ToString();
            txt_H_Aris.Text = DGV_Aris_todo.SelectedRows[0].Cells[3].Value.ToString();
            txt_glaa_Aris.Text = DGV_Aris_todo.SelectedRows[0].Cells[4].Value.ToString();
            txt_track_ID.Text = DGV_Aris_todo.SelectedRows[0].Cells[7].Value.ToString();
            txt_qtysend_Aris.Focus();
        }
            ...
            else
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    var filter = from fil in trackdb.Tracks
                                 ...
                                 where fil.Departmet_ID == 13 && fil.QTY_ToDo > 0 && fil.OC_ID == int.Parse(txtOrderFilter.Text)
                    ...
                    DGV_Aris_todo.DataSource = filter;
                }
            }
        }
        ...
        private void btnSelectItem_Click(object sender, EventArgs e)
        {
            ...
            {
                for (int i = 0; i < DGV_Aris_todo.SelectedRows.Count; i++)
                {
                    var FullOrder = from up in trackdb.Tracks
                                    join iddd in trackdb.ITEMs on new { up.OC_ID, up.Item_ID, up.Pos } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos }
                                    join dep in trackdb.Departments on up.Departmet_ID equals dep.Departmet_ID
                                    where up.Track_ID == int.Parse(DGV_Aris_todo.SelectedRows[i].Cells[7].Value.ToString())
                    ...
                    foreach (Track up in FullOrder)
                    {
                        ...
                        var Next = (from Ne in trackdb.Tracks
                                    join iddd in trackdb.ITEMs on new { Ne.OC_ID, Ne.Item_ID, Ne.Pos, Ne.Glass_ID } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos, iddd.Glass_ID }
                                    where Ne.Track_ID == int.Parse(DGV_Aris_todo.SelectedRows[i].Cells[7].Value.ToString())
                                    select new { DepID = Ne.Departmet_ID, step1 = iddd.Step1, step2 = iddd.Step2, step3 = iddd.Step3, step4 = iddd.Step4, step5 = iddd.Step5, step6 = iddd.Step6, step7 = iddd.Step7, step8 = iddd.Step8, }).SingleOrDefault();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                var check = (from ch in trackdb.DepDatas
                             where ch.Date == dateCut.Value && ch.Department_ID == 4 && ch.Shift_ID == Shift.ID
        ...
        private void comboSV_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void DGV_Aris_todo_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
