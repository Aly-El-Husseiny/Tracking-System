...
namespace Cutting
{
    public partial class frmcut : Form
    {
        ...
        private void frmcut_Load(object sender, EventArgs e)
        {
            ...
            comboSV.DataSource = trackdb.Logins.Where(x => x.Departmet_ID == 1).ToList();
        ...
        private void RefreshCut()
        {
            ...
            var cut = from id in trackdb.Tracks
                      ...
                      //  where id.Departmet_ID == 1 && id.QTY_ToDo > 0 && (id.Recived_From == 8 || id.Recived_From == 10)  Stock
                      where id.Departmet_ID == 1 && id.QTY_ToDo > 0// && id.Recived_From ==8
            ...
            for (int i = 0; i < DGV_cut_todo.Rows.Count; ++i)
            {
                sumqty_grind += Convert.ToInt32(DGV_cut_todo.Rows[i].Cells[5].Value);
            ...
            for (int i = 0; i < DGV_cut_todo.Rows.Count; ++i)
            {
                int qty = Convert.ToInt32(DGV_cut_todo.Rows[i].Cells[5].Value);
                double w = Convert.ToDouble(DGV_cut_todo.Rows[i].Cells[2].Value);
                double h = Convert.ToDouble(DGV_cut_todo.Rows[i].Cells[3].Value);
        ...
        private void btn_Send_Click(object sender, EventArgs e)
        {
            ...
            if (txt_qtysend_cut.Text == "" || comboShift.Text == "" || comboSV.Text == "" || comboQC.Text == "" || comboQA.Text == "")
            ...
            else
            {
                ...
                foreach (Track up in work)
                {
                    ...
                    else
                    {
                        ...
                        /////////////////////////////////////////
                        var glasscost = (from co in trackdb.GlassTypes where co.Glass_ID == up.Glass_ID select new { co.Cutting_Cost }).SingleOrDefault();
                        //////
                        Track add = new Track();
                        add.OC_ID = up.OC_ID;
                        ...
                        add.QTY_Recive = int.Parse(txt_qtysend_cut.Text);
                        add.QTY_ToDo = int.Parse(txt_qtysend_cut.Text);
                        ...
                        ///////////
                        add.Standard_Unit_Process_Cost = glasscost.Cutting_Cost;
                        add.ProcessType = 19;
        ...
        private void btnFinishcut_Click(object sender, EventArgs e)
        {
            var track = from id in trackdb.Tracks
                        ...
                        where (id.Date <= dateTocut.Value && id.Date >= datefromcut.Value) && id.Recived_From == 1
                        select new
                        {
                            ...
                            //SQM_Send= (iddd.Width* iddd.Hieght * id.QTY_Recive)/1000000,
            ...
            {
                sumqty_Finish += Convert.ToInt32(DGV_Grind_Detail.Rows[i].Cells[5].Value);
            ...
            else
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    var filter = from fil in trackdb.Tracks
                                 ...
                                 where fil.Departmet_ID == 1 && fil.QTY_ToDo > 0 && fil.OC_ID == int.Parse(txtOrderFilter.Text)
                    ...
                    DGV_cut_todo.DataSource = filter;
                }
            }
        }
            ...
            else
            {
                for (int i = 0; i < DGV_cut_todo.SelectedRows.Count; i++)
                {
                    ...
                    foreach (Track up in FullOrder)
                    {
                        ...
                        var Next = (from Ne in trackdb.Tracks
                                    join iddd in trackdb.ITEMs on new { Ne.OC_ID, Ne.Item_ID, Ne.Pos, Ne.Glass_ID } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos, iddd.Glass_ID }
                                    where Ne.Track_ID == int.Parse(DGV_cut_todo.SelectedRows[i].Cells[7].Value.ToString())
                                    select new { DepID = Ne.Departmet_ID, step1 = iddd.Step1, step2 = iddd.Step2, step3 = iddd.Step3, step4 = iddd.Step4, step5 = iddd.Step5, step6 = iddd.Step6, step7 = iddd.Step7, step8 = iddd.Step8, }).SingleOrDefault();
        ...
        private void DGV_cut_todo_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            txt_oc_cut.Text = DGV_cut_todo.SelectedRows[0].Cells[0].Value.ToString();
            txt_item_cut.Text = DGV_cut_todo.SelectedRows[0].Cells[1].Value.ToString();
            txt_w_cut.Text = DGV_cut_todo.SelectedRows[0].Cells[2].Value.ToString();
        ...
        private void comboGLassStock_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void button1_Click_2(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                var check = (from ch in trackdb.DepDatas
                             where ch.Date == dateCut.Value && ch.Department_ID == 1 && ch.Shift_ID == Shift.ID
        ...
        private void DGV_cut_todo_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {

        }

        private void txt_qtysend_cut_KeyPress_1(object sender, KeyPressEventArgs e)
        {
