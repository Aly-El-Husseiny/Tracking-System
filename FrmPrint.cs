    ...
    public partial class FrmPrint : Form
    {
        ...
        private void RefreshPrint()
        {
            ...
            var cut = from id in trackdb.Tracks
                      ...
                      where id.Departmet_ID == 3 && id.QTY_ToDo > 0
            ...
            DGV_Print_todo.DataSource = cut;

            trackdb.SubmitChanges();

            DGV_Print_todo.Columns[5].DefaultCellStyle.BackColor = Color.Pink;
            for (int i = 0; i < DGV_Print_todo.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_Print_todo.Rows[i].DefaultCellStyle.BackColor = Color.LightBlue;
                }
            }
            ...
            for (int i = 0; i < DGV_Print_todo.Rows.Count; ++i)
            {
                sumqty_grind += Convert.ToInt32(DGV_Print_todo.Rows[i].Cells[5].Value);
            ...
            for (int i = 0; i < DGV_Print_todo.Rows.Count; ++i)
            {
                int qty = Convert.ToInt32(DGV_Print_todo.Rows[i].Cells[5].Value);
                double w = Convert.ToDouble(DGV_Print_todo.Rows[i].Cells[2].Value);
                double h = Convert.ToDouble(DGV_Print_todo.Rows[i].Cells[3].Value);
                double raw1 = (qty * w * h) / 1000000;
                sumArea_grind += Convert.ToDouble(raw1);

            }
            txt_tarea.Text = sumArea_grind.ToString();
        }
        ...
        private void FrmPrint_Load(object sender, EventArgs e)
        {
            ...
            comboSV.DataSource = trackdb.Logins.Where(x => x.Departmet_ID == 3).ToList();
        ...
        private void btn_send_print_Click(object sender, EventArgs e)
        {
            ...
            if (txt_qtysend_Print.Text == "" || comboShift.Text == "" || comboSV.Text == "" || comboQC.Text == "" || comboQA.Text == "")
            ...
            else
            {
                ...
                {
                    if (up.QTY_ToDo < int.Parse(txt_qtysend_Print.Text))
                    { MessageBox.Show(@"الكمية المدخلة أكبر من المطلوب"); }
                    else
                    {
                        up.QTY_ToDo = up.QTY_ToDo - int.Parse(txt_qtysend_Print.Text);
                        up.QTY_Send = int.Parse(txt_qtysend_Print.Text);
                        ...
                        /////////////////////////////////////////
                        var PrintType = (from ty in trackdb.Orders where ty.OC_ID == up.OC_ID select new { ty.Print_type }).SingleOrDefault();
                        var printcost = (from pco in trackdb.Processes where pco.ID == PrintType.Print_type select new { pco.Pocess_Cost }).SingleOrDefault();
                        /////
                        ...
                        add.QTY_Recive = int.Parse(txt_qtysend_Print.Text);
                        add.QTY_ToDo = int.Parse(txt_qtysend_Print.Text);
                        ...
                        ///////////
                        add.ProcessType = PrintType.Print_type;
                        add.Standard_Unit_Process_Cost = printcost.Pocess_Cost;
        ...
        private void btnFinishPrint_Click(object sender, EventArgs e)
        {
            var track = from id in trackdb.Tracks
                        join idd in trackdb.GlassTypes on id.Glass_ID equals idd.Glass_ID
                        join iddd in trackdb.ITEMs on new { id.OC_ID, id.Item_ID, id.Pos } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos }
                        join idDEP in trackdb.Departments on id.Departmet_ID equals idDEP.Departmet_ID
                        where (id.Date <= dateToPrint.Value && id.Date >= datefromPrint.Value) && id.Recived_From == 3
                        select new
                        {
            ...
            DGV_Print_Detail.DataSource = track;

            for (int i = 0; i < DGV_Print_Detail.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_Print_Detail.Rows[i].DefaultCellStyle.BackColor = Color.LightBlue;
                }
            }
        ...
        private void copyAlltoClipboard()
        {
            ...
            DGV_Print_Detail.ClipboardCopyMode = DataGridViewClipboardCopyMode.EnableAlwaysIncludeHeaderText;
            DGV_Print_Detail.MultiSelect = true;
            DGV_Print_Detail.SelectAll();
            DataObject dataObj = DGV_Print_Detail.GetClipboardContent();
            if (dataObj != null)
        ...
        private void DGV_Print_todo_DoubleClick(object sender, EventArgs e)
        {
            txt_oc_Print.Text = DGV_Print_todo.SelectedRows[0].Cells[0].Value.ToString();
            txt_item_Print.Text = DGV_Print_todo.SelectedRows[0].Cells[1].Value.ToString();
            txt_w_Print.Text = DGV_Print_todo.SelectedRows[0].Cells[2].Value.ToString();
            txt_H_Print.Text = DGV_Print_todo.SelectedRows[0].Cells[3].Value.ToString();
            txt_glaa_Print.Text = DGV_Print_todo.SelectedRows[0].Cells[4].Value.ToString();
            txt_track_ID.Text = DGV_Print_todo.SelectedRows[0].Cells[7].Value.ToString();
            txt_qtysend_Print.Focus();
        }

        private void txt_qtysend_Print_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    var filter = from fil in trackdb.Tracks
                                 ...
                                 where fil.Departmet_ID == 3 && fil.QTY_ToDo > 0 && fil.OC_ID == int.Parse(txtOrderFilter.Text)
                    ...
                    DGV_Print_todo.DataSource = filter;
                }
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                var check = (from ch in trackdb.DepDatas
                             where ch.Date == dateCut.Value && ch.Department_ID == 3 && ch.Shift_ID == Shift.ID
        ...
        private void btnSelectItem_Click(object sender, EventArgs e)
        {
            ...
            {
                for (int i = 0; i < DGV_Print_todo.SelectedRows.Count; i++)
                {
                    var FullOrder = from up in trackdb.Tracks
                                    join iddd in trackdb.ITEMs on new { up.OC_ID, up.Item_ID, up.Pos } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos }
                                    join dep in trackdb.Departments on up.Departmet_ID equals dep.Departmet_ID
                                    where up.Track_ID == int.Parse(DGV_Print_todo.SelectedRows[i].Cells[7].Value.ToString())
                    ...
                    foreach (Track up in FullOrder)
                    {
                        ...
                        var Next = (from Ne in trackdb.Tracks
                                    join iddd in trackdb.ITEMs on new { Ne.OC_ID, Ne.Item_ID, Ne.Pos, Ne.Glass_ID } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos, iddd.Glass_ID }
                                    where Ne.Track_ID == int.Parse(DGV_Print_todo.SelectedRows[i].Cells[7].Value.ToString())
                                    select new { DepID = Ne.Departmet_ID, step1 = iddd.Step1, step2 = iddd.Step2, step3 = iddd.Step3, step4 = iddd.Step4, step5 = iddd.Step5, step6 = iddd.Step6, step7 = iddd.Step7, step8 = iddd.Step8, }).SingleOrDefault();
