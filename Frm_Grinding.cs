...
namespace Cutting
{
    public partial class frmgrind : Form
    {
        ...
        private void frmgrind_Load(object sender, EventArgs e)
        {
            ...
            comboSV.DataSource = trackdb.Logins.Where(x => x.Departmet_ID == 2).ToList();
        ...
        {
            System.Diagnostics.Process.Start("explorer.exe", @"\\SERVER\Work Orders_From 40000");

        }

        private void DGV_Grind_todo_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            txt_oc_grind.Text = DGV_Grind_todo.SelectedRows[0].Cells[2].Value.ToString();

        }

        private void Refreshgrind()
        {
            DGV_Grind_todo.DataSource = "";
            ...
            var grind = from id in trackdb.Tracks
                        ...
                        where id.Departmet_ID == 2 && id.QTY_ToDo > 0
            ...
            DGV_Grind_todo.DataSource = grind;

            trackdb.SubmitChanges();

            DGV_Grind_todo.Columns[5].DefaultCellStyle.BackColor = Color.Pink;
            for (int i = 0; i < DGV_Grind_todo.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_Grind_todo.Rows[i].DefaultCellStyle.BackColor = Color.LightBlue;
                }
            }
            ...
            for (int i = 0; i < DGV_Grind_todo.Rows.Count; ++i)
            {
                sumqty_grind += Convert.ToInt32(DGV_Grind_todo.Rows[i].Cells[5].Value);
            ...
            for (int i = 0; i < DGV_Grind_todo.Rows.Count; ++i)
            {
                int qty = Convert.ToInt32(DGV_Grind_todo.Rows[i].Cells[5].Value);
                double w = Convert.ToDouble(DGV_Grind_todo.Rows[i].Cells[2].Value);
                double h = Convert.ToDouble(DGV_Grind_todo.Rows[i].Cells[3].Value);
                double raw1 = (qty * w * h) / 1000000;
                sumArea_grind += Convert.ToDouble(raw1);

            }
            txt_tarea_grind.Text = sumArea_grind.ToString();
        }
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            var track = from id in trackdb.Tracks
                        join idd in trackdb.GlassTypes on id.Glass_ID equals idd.Glass_ID
                        join iddd in trackdb.ITEMs on new { id.OC_ID, id.Item_ID } equals new { iddd.OC_ID, iddd.Item_ID }
                        join idDEP in trackdb.Departments on id.Departmet_ID equals idDEP.Departmet_ID
                        where (id.Date <= dateToGrind.Value && id.Date >= datefromGrind.Value) && id.Recived_From == 2
                        select new
                        {
            ...
            DGV_Grind_Detail.DataSource = track;
        ...
        private void dateGrind_Leave(object sender, EventArgs e)
        {

            
        }

        private void DGV_Grind_todo_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
           
        }

        private void DGV_Grind_todo_MouseDoubleClick_1(object sender, MouseEventArgs e)
        {

            txt_oc_grind.Text = DGV_Grind_todo.SelectedRows[0].Cells[0].Value.ToString();
            txt_item_grind.Text = DGV_Grind_todo.SelectedRows[0].Cells[1].Value.ToString();
            txt_w_grind.Text = DGV_Grind_todo.SelectedRows[0].Cells[2].Value.ToString();
            txt_H_grind.Text = DGV_Grind_todo.SelectedRows[0].Cells[3].Value.ToString();
            txt_glaa_grind.Text = DGV_Grind_todo.SelectedRows[0].Cells[4].Value.ToString();
            txt_track_ID.Text = DGV_Grind_todo.SelectedRows[0].Cells[7].Value.ToString();
            txt_qtysend_grind.Focus();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (txt_qtysend_grind.Text == "" ||txt_oc_grind.Text=="" || comboShift.Text == "" || comboSV.Text == "" || comboQC.Text == "" || comboQA.Text == "")
            { MessageBox.Show(@"من فضلك ادخل الكمية المرسلة "); }
            ...
            else
            {
                ...
                {
                    if (up.QTY_ToDo < int.Parse(txt_qtysend_grind.Text))
                    { MessageBox.Show(@"الكمية المدخلة أكبر من المطلوب"); }
                    else
                    {
                        up.QTY_ToDo = up.QTY_ToDo - int.Parse(txt_qtysend_grind.Text);
                        up.QTY_Send = int.Parse(txt_qtysend_grind.Text);
                        ////////// Send to Next ///////////////////
                        ...
                        /////////////////////////////////////////
                        var glassfremless = (from fr in trackdb.GlassTypes where fr.Glass_ID == up.Glass_ID select new { fr.Frameless_Cost }).SingleOrDefault();
                        var glasspolish = (from po in trackdb.GlassTypes where po.Glass_ID == up.Glass_ID select new { po.Polishing_Cost }).SingleOrDefault();
                        var grindType = (from ty in trackdb.Orders where ty.OC_ID == up.OC_ID select new { ty.Grind_Type }).SingleOrDefault();
                        /////
                        Track add = new Track();
                        add.OC_ID = up.OC_ID;
                        ...
                        add.QTY_Recive = int.Parse(txt_qtysend_grind.Text);
                        add.QTY_ToDo = int.Parse(txt_qtysend_grind.Text);
                        //add.QTY_Send = 0:
                        ...
                        if (grindType.Grind_Type == 16) { add.ProcessType = 16; add.Standard_Unit_Process_Cost = glassfremless.Frameless_Cost; }
                        else  { add.ProcessType = 17; add.Standard_Unit_Process_Cost = glasspolish.Polishing_Cost; }
                        ...
                        { add.Furnace = "Automatic"; }
                        else { add.Furnace = "Manual"; }
        ...
        private void txt_qtysend_grind_KeyPress_1(object sender, KeyPressEventArgs e)
        {
        ...
        private void copyAlltoClipboard()
        {
            ...
            DGV_Grind_Detail.ClipboardCopyMode = DataGridViewClipboardCopyMode.EnableAlwaysIncludeHeaderText;
            DGV_Grind_Detail.MultiSelect = true;
            DGV_Grind_Detail.SelectAll();
            DataObject dataObj = DGV_Grind_Detail.GetClipboardContent();
            if (dataObj != null)
        ...
        private void txt_oc_grind_TextChanged(object sender, EventArgs e)
        {

        }

        private void txt_Grind_Man_TextChanged(object sender, EventArgs e)
        {

        }

        private void txt_Grind_Man_KeyPress(object sender, KeyPressEventArgs e)
        {
            Utility.onlynumber(e);
        }

        private void txt_grind_prodT_TextChanged(object sender, EventArgs e)
        {

        }

        private void txt_grind_prodT_KeyPress(object sender, KeyPressEventArgs e)
        {
            Utility.onlynumber(e);
        }

        private void txt_grind_downT_KeyPress(object sender, KeyPressEventArgs e)
        {
            Utility.onlynumber(e);
        }
        ...
        private void btnFinishGrind_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < DGV_Grind_Detail.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_Grind_Detail.Rows[i].DefaultCellStyle.BackColor = Color.LightBlue;
                }
            }
        ...
        private void btnExcelGrind_Click(object sender, EventArgs e)
        {
            copyAlltoClipboard();
            Microsoft.Office.Interop.Excel.Application xlexcel;
            Microsoft.Office.Interop.Excel.Workbook xlWorkBook;
        ...
        private void txtOrderFilter_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    var filter = from fil in trackdb.Tracks
                                 ...
                                 where fil.Departmet_ID == 2 && fil.QTY_ToDo > 0 && fil.OC_ID == int.Parse(txtOrderFilter.Text)
                    ...
                    DGV_Grind_todo.DataSource = filter;
                }
            }
        }
        ...
        private void btnSelectItem_Click(object sender, EventArgs e)
        {
                ...
                for (int i = 0; i < DGV_Grind_todo.SelectedRows.Count; i++)
                {
                    var FullOrder = from up in trackdb.Tracks
                                    join iddd in trackdb.ITEMs on new { up.OC_ID, up.Item_ID, up.Pos } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos }
                                    join dep in trackdb.Departments on up.Departmet_ID equals dep.Departmet_ID
                                    where up.Track_ID == int.Parse(DGV_Grind_todo.SelectedRows[i].Cells[7].Value.ToString())
                    ...
                    var Next = (from Ne in trackdb.Tracks
                                join iddd in trackdb.ITEMs on new { Ne.OC_ID, Ne.Item_ID, Ne.Pos, Ne.Glass_ID } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos, iddd.Glass_ID }
                                where Ne.Track_ID == int.Parse(DGV_Grind_todo.SelectedRows[i].Cells[7].Value.ToString())
                                select new { DepID = Ne.Departmet_ID, step1 = iddd.Step1, step2 = iddd.Step2, step3 = iddd.Step3, step4 = iddd.Step4, step5 = iddd.Step5, step6 = iddd.Step6, step7 = iddd.Step7, step8 = iddd.Step8, }).SingleOrDefault();
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                var check = (from ch in trackdb.DepDatas
                             where ch.Date == dateGrind.Value && ch.Department_ID ==2 && ch.Shift_ID == Shift.ID
