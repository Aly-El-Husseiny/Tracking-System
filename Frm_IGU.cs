...
namespace Cutting
{
    public partial class Frm_IGU : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            var work = from up in trackdb.Tracks
                       ...
                       where up.Track_ID == int.Parse(txt_Track1.Text)

                       select up;

            if (txt_IGU.Text.StartsWith("IGU"))
            {
                var IGU2 = from up2 in trackdb.Tracks
                           ...
                           where up2.Track_ID == int.Parse(txt_Track2.Text)

                           select up2;

                foreach (Track up in work)
                {
                    foreach (Track up2 in IGU2)
                    {

                        if (((up.QTY_ToDo < int.Parse(txt_qtysend_IGU.Text)) || (up2.QTY_ToDo < int.Parse(txt_qtysend_IGU.Text))) || txt_qtysend_IGU.Text == "" )

                        { MessageBox.Show(@"من فضلك تأكد من البيانات المدخلة"); }
                        else
                        {
                            up.QTY_ToDo = up.QTY_ToDo - int.Parse(txt_qtysend_IGU.Text);
                            up.QTY_Send = int.Parse(txt_qtysend_IGU.Text);
                            ...
                            var Next = (from Ne in trackdb.Tracks
                                        ...
                                        where Ne.Track_ID == int.Parse(txt_Track1.Text)
                            ...
                            /////////////////////////////////////////
                            var IGUType = (from ty in trackdb.Orders where ty.OC_ID == up.OC_ID select new { ty.IGU_type }).SingleOrDefault();
                            var IGUcost = (from pco in trackdb.Processes where pco.ID == IGUType.IGU_type select new { pco.Pocess_Cost }).SingleOrDefault();
                            /////////////////////////////////////////
                            Track add = new Track();
                            add.OC_ID = up.OC_ID;
                            ...
                            add.IGU = txt_IGU.Text;
                            add.QTY_Recive = int.Parse(txt_qtysend_IGU.Text);

                            add.Standard_Unit_Process_Cost = IGUcost.Pocess_Cost;
                            add.ProcessType = IGUType.IGU_type;

                            add.QTY_ToDo = int.Parse(txt_qtysend_IGU.Text);
                            ...
                            up2.QTY_ToDo = up2.QTY_ToDo - int.Parse(txt_qtysend_IGU.Text);
                            up2.QTY_Send += int.Parse(txt_qtysend_IGU.Text);
            ...
            else if (txt_IGU.Text.StartsWith("Bonding"))
            {
                foreach (Track up in work)
                {
                    if ((up.QTY_ToDo < int.Parse(txt_qtysend_IGU.Text)) || txt_qtysend_IGU.Text == "" )
                    { MessageBox.Show(@"من فضلك تأكد من البيانات المدخلة"); }
                    else
                    {
                        up.QTY_ToDo = up.QTY_ToDo - int.Parse(txt_qtysend_IGU.Text);
        ...
        private void RefreshIGU()
        {
            DGV_IGU_todo.DataSource = "";
            //depname = trackdb.Departments.Single(IDP => IDP.Departmet_ID == id.Recived_From)

            var IGU = from id in trackdb.Tracks
                        ...
                        where id.Departmet_ID == 6 && id.QTY_ToDo > 0 && id.Recived_From !=5
            ...
            DGV_IGU_todo.DataSource = IGU;

            trackdb.SubmitChanges();

            DGV_IGU_todo.Columns[5].DefaultCellStyle.BackColor = Color.Pink;
            for (int i = 0; i < DGV_IGU_todo.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_IGU_todo.Rows[i].DefaultCellStyle.BackColor = Color.LightBlue;
                }
            }
            ...
            for (int i = 0; i < DGV_IGU_todo.Rows.Count; ++i)
            {
                sumqty_grind += Convert.ToInt32(DGV_IGU_todo.Rows[i].Cells[5].Value);
            ...
            for (int i = 0; i < DGV_IGU_todo.Rows.Count; ++i)
            {
                int qty = Convert.ToInt32(DGV_IGU_todo.Rows[i].Cells[5].Value);
                double w = Convert.ToDouble(DGV_IGU_todo.Rows[i].Cells[2].Value);
                double h = Convert.ToDouble(DGV_IGU_todo.Rows[i].Cells[3].Value);
                double raw1 = (qty * w * h) / 1000000;
                sumArea_grind += Convert.ToDouble(raw1);

            }
            txt_tarea_IGU.Text = sumArea_grind.ToString();
        }
        private void Frm_IGU_Load(object sender, EventArgs e)
        {
            ...
            comboSV.DataSource = trackdb.Logins.Where(x => x.Departmet_ID == 6).ToList();
        ...
        private void DGV_IGU_todo_DoubleClick(object sender, EventArgs e)
        {
            if (txt_oc_IGU1.Text == "")
            {
                txt_oc_IGU1.Text = DGV_IGU_todo.SelectedRows[0].Cells[0].Value.ToString();
                txt_item_IGU1.Text = DGV_IGU_todo.SelectedRows[0].Cells[1].Value.ToString();
                txt_w_IGU1.Text = DGV_IGU_todo.SelectedRows[0].Cells[2].Value.ToString();
                txt_h_IGU1.Text = DGV_IGU_todo.SelectedRows[0].Cells[3].Value.ToString();
                txt_glass_IGU1.Text = DGV_IGU_todo.SelectedRows[0].Cells[4].Value.ToString();
                txt_Track1.Text = DGV_IGU_todo.SelectedRows[0].Cells[7].Value.ToString();
                txtPOS1.Text= DGV_IGU_todo.SelectedRows[0].Cells[9].Value.ToString();
            }
           else if (txt_oc_IGU1.Text != ""&& txt_oc_IGU2.Text=="")
            {
                txt_oc_IGU2.Text = DGV_IGU_todo.SelectedRows[0].Cells[0].Value.ToString();
                txt_item_IGU2.Text = DGV_IGU_todo.SelectedRows[0].Cells[1].Value.ToString();
                txt_w_IGU2.Text = DGV_IGU_todo.SelectedRows[0].Cells[2].Value.ToString();
                txt_h_IGU2.Text = DGV_IGU_todo.SelectedRows[0].Cells[3].Value.ToString();
                txt_glass_IGU2.Text = DGV_IGU_todo.SelectedRows[0].Cells[4].Value.ToString();
                txt_Track2.Text = DGV_IGU_todo.SelectedRows[0].Cells[7].Value.ToString();
                txtPOS2.Text = DGV_IGU_todo.SelectedRows[0].Cells[9].Value.ToString();
            }
        }

        private void btnFinishIGU_Click(object sender, EventArgs e)
        {
            var IGU = from id in trackdb.Tracks
                        ...
                        where id.Date <= dateToIGU.Value && id.Date >= datefromIGU.Value && id.Recived_From == 6
                        select new
                        {
                            ...
                            Width = iddd.Width,
                            Height = iddd.Hieght,
                            GlassType = idd.Descreption,
                            Qty_Send = id.QTY_Recive,
                            Send_To = idDEP.Department_Name,
                            Balance = id.Balance,
                            Trak_ID = id.Track_ID
                        };
            DGV_IGU_Detail.DataSource = IGU;

            for (int i = 0; i < DGV_IGU_Detail.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_IGU_Detail.Rows[i].DefaultCellStyle.BackColor = Color.LightBlue;
                }
            }
        ...
        private void finishToText()
            {
            // Finish to textbox
            int sumqty_Finish = 0;
            for (int i = 0; i < DGV_IGU_Detail.Rows.Count; ++i)
            {
                sumqty_Finish += Convert.ToInt32(DGV_IGU_Detail.Rows[i].Cells[5].Value);

            }
            txt_T_Finish_QTY.Text = sumqty_Finish.ToString();

            double sumArea_Finish = 0;
            for (int i = 0; i < DGV_IGU_Detail.Rows.Count; ++i)
            {
                int qty = Convert.ToInt32(DGV_IGU_Detail.Rows[i].Cells[5].Value);
                double w = Convert.ToDouble(DGV_IGU_Detail.Rows[i].Cells[2].Value);
                double h = Convert.ToDouble(DGV_IGU_Detail.Rows[i].Cells[3].Value);
                double raw1 = (qty * w * h) / 1000000;
                sumArea_Finish += Convert.ToDouble(raw1);

            }
            txt_T_Finish_SQM.Text = sumArea_Finish.ToString();
            /////
        ...
        private void copyAlltoClipboard()
        {
            ...
            DGV_IGU_Detail.ClipboardCopyMode = DataGridViewClipboardCopyMode.EnableAlwaysIncludeHeaderText;
            DGV_IGU_Detail.MultiSelect = true;
            DGV_IGU_Detail.SelectAll();
            DataObject dataObj = DGV_IGU_Detail.GetClipboardContent();
            if (dataObj != null)
                Clipboard.SetDataObject(dataObj);

        }
        private void btnExcelIGU_Click(object sender, EventArgs e)
        {
            copyAlltoClipboard();
        ...
        private void toolStripLG_Click(object sender, EventArgs e)
        {
            ...
            var IGU = from id in trackdb.Tracks
                      ...
                      join iddd in trackdb.ITEMs on new { id.OC_ID, id.Item_ID, id.Pos } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos }
                      join depar in trackdb.Departments on id.Recived_From equals depar.Departmet_ID
                      where id.Departmet_ID == 6 && id.QTY_ToDo > 0 && (id.Recived_From==5 || id.Recived_From == 9)
            ...
            for (int i = 0; i < DGV_IGU_todo.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_IGU_todo.Rows[i].DefaultCellStyle.BackColor = Color.LightPink;
                }
            }
        ...
        {
            if (txt_oc_IGU1.Text == "" || txt_oc_IGU2.Text == "" || txt_oc_IGU1.Text != txt_oc_IGU2.Text 
                || txt_item_IGU1.Text != txt_item_IGU2.Text ||txt_Track1.Text==txt_Track2.Text || txtPOS1.Text==txtPOS2.Text )
            {
                MessageBox.Show(@"من فضلك اختار نوع الزجاج الصحيح");
            }
            
            else
            {

                txt_IGU.Text = "IGU(" + txt_glass_IGU1.Text + "+" + "A/S"+"+" + txt_glass_IGU2.Text + ")";
                btn_send.Enabled = true;
            }
            txt_qtysend_IGU.Focus();
        }

        private void txt_qtysend_IGU_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    var filter = from fil in trackdb.Tracks
                                 ...
                                 where fil.Departmet_ID == 6 && fil.QTY_ToDo > 0 && fil.OC_ID == int.Parse(txtOrderFilter.Text)
                    ...
                    DGV_IGU_todo.DataSource = filter;
                }
            }
        }

        private void btn_bonding_Click(object sender, EventArgs e)
        {
            txt_IGU.Text = "Bonding to frame ( " + txt_glass_IGU1.Text + " )";
            btn_send.Enabled = true;
            txt_qtysend_IGU.Focus();
        }
        ...
        private void txtFilterFinish_KeyDown(object sender, KeyEventArgs e)
        {
            if (txtFilterFinish.Text == "" && e.KeyCode == Keys.Enter)
            { MessageBox.Show(@"Please enter the Order No."); }
            else
            {
                ...
                {
                    var filter = from fil in trackdb.Tracks
                                 join idd in trackdb.Orders on fil.OC_ID equals idd.OC_ID
                                 join iddd in trackdb.ITEMs on new { fil.OC_ID, fil.Item_ID, fil.Pos } equals new { iddd.OC_ID, iddd.Item_ID, iddd.Pos }
                                 join depar in trackdb.Departments on fil.Recived_From equals depar.Departmet_ID
                                 where fil.Date <= dateToIGU.Value && fil.Date >= datefromIGU.Value && fil.Recived_From == 6 && fil.OC_ID==int.Parse(txtFilterFinish.Text)
                                 select new
                                 {
                                     ...
                                     Height = iddd.Hieght,
                                     GlassType = idd.Descreption,
                                     Qty_Send = fil.QTY_Recive,
                                     Send_To = depar.Department_Name,
                                     Balance = fil.Balance,
                                     Trak_ID = fil.Track_ID
                                 };
               

                    DGV_IGU_Detail.DataSource = filter;
                    finishToText();
                }
            }
        }

        private void txtFilterFinish_KeyPress(object sender, KeyPressEventArgs e)
        {
            Utility.onlynumber(e);
        }

        private void btncleardep_Click_1(object sender, EventArgs e)
        {
            txt_cut_Man.Text = "";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                var check = (from ch in trackdb.DepDatas
                             where ch.Date == dateCut.Value && ch.Department_ID == 6 && ch.Shift_ID == Shift.ID
