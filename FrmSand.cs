...
namespace Cutting
{
    public partial class FrmSand : Form
    {
        ...
        private void RefreshPrint()
        {
            ...
            var cut = from id in trackdb.Tracks
                      ...
                      where id.Departmet_ID == 31 && id.QTY_ToDo > 0
            ...
            DGV_Sand_todo.DataSource = cut;

            trackdb.SubmitChanges();

            DGV_Sand_todo.Columns[5].DefaultCellStyle.BackColor = Color.Pink;
            for (int i = 0; i < DGV_Sand_todo.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    DGV_Sand_todo.Rows[i].DefaultCellStyle.BackColor = Color.LightBlue;
                }
            }
            ...
            for (int i = 0; i < DGV_Sand_todo.Rows.Count; ++i)
            {
                sumqty_grind += Convert.ToInt32(DGV_Sand_todo.Rows[i].Cells[5].Value);
            ...
            for (int i = 0; i < DGV_Sand_todo.Rows.Count; ++i)
            {
                int qty = Convert.ToInt32(DGV_Sand_todo.Rows[i].Cells[5].Value);
                double w = Convert.ToDouble(DGV_Sand_todo.Rows[i].Cells[2].Value);
                double h = Convert.ToDouble(DGV_Sand_todo.Rows[i].Cells[3].Value);
        ...
        private void btn_send_print_Click(object sender, EventArgs e)
        {
            ...
            if (txt_qtysend_Sand.Text == "" || comboShift.Text == "" || comboSV.Text == "" || comboQC.Text == "" || comboQA.Text == "")
            ...
            else
            {
                ...
                {
                    if (up.QTY_ToDo < int.Parse(txt_qtysend_Sand.Text))
                    { MessageBox.Show(@"الكمية المدخلة أكبر من المطلوب"); }
                    else
                    {
                        up.QTY_ToDo = up.QTY_ToDo - int.Parse(txt_qtysend_Sand.Text);
                        up.QTY_Send = int.Parse(txt_qtysend_Sand.Text);
                        ...
                        add.QTY_Recive = int.Parse(txt_qtysend_Sand.Text);
                        add.QTY_ToDo = int.Parse(txt_qtysend_Sand.Text);
        ...
        private void btnFinishPrint_Click(object sender, EventArgs e)
        {
            var track = from id in trackdb.Tracks
                        ...
                        where (id.Date <= dateToPrint.Value && id.Date >= datefromPrint.Value) && id.Recived_From == 31
                        select new
                        {
        ...
        {
            txt_oc_Sand.Text = DGV_Sand_todo.SelectedRows[0].Cells[0].Value.ToString();
            txt_item_Sand.Text = DGV_Sand_todo.SelectedRows[0].Cells[1].Value.ToString();
            txt_w_Sand.Text = DGV_Sand_todo.SelectedRows[0].Cells[2].Value.ToString();
            txt_H_Sand.Text = DGV_Sand_todo.SelectedRows[0].Cells[3].Value.ToString();
            txt_glaa_Sand.Text = DGV_Sand_todo.SelectedRows[0].Cells[4].Value.ToString();
            txt_track_ID.Text = DGV_Sand_todo.SelectedRows[0].Cells[7].Value.ToString();
            txt_qtysend_Sand.Focus();
        }
            ...
            else
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    var filter = from fil in trackdb.Tracks
                                 ...
                                 where fil.Departmet_ID == 31 && fil.QTY_ToDo > 0 && fil.OC_ID == int.Parse(txtOrderFilter.Text)
                    ...
                    DGV_Sand_todo.DataSource = filter;
                }
            }
        }
